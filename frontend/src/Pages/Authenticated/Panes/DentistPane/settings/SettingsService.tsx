...
} from "@/API/Authenticated/Dentist/SettingsAPI";
import {
  Save,
  AlertCircle,
  CheckCircle2,
...
export default function SettingsService() {
    ...
    (async () => {
      try {
        const [svcRes, dentRes] = await Promise.all([
          getServices(),
          getDentistServices(userId),
        ]);
        setAllServices(svcRes.data ?? []);

        const ids =
          dentRes.dentistServices?.map((d: DentistService) => d.service_id) ??
          [];
        setOriginalIds(ids);
  ...
  return (
    <SettingsSection
      ...
      description="Select the procedures you are qualified to perform."
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          {Object.entries(grouped).map(([type, list]) => {
            const checkedCount = list.filter((s) =>
              selected.has(s.service_id),
            ).length;
            ...
            return (
              <div
                key={type}
                className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden hover:border-indigo-100 transition-colors"
              >
                    ...
                    return (
                      ...
                      >
                        <input
                          ...
                          checked={isChecked}
                          onChange={(e) =>
                            toggle(s.service_id, e.target.checked)
                          }
