...
import {
  getDentistServices,
  createDentistService,
  updateDentistService,
  deleteDentistService,
} from "@/API/Authenticated/admin/DentistService";
...
export default function DentistService() {
  ...
  const uniqueServiceNames = [
    "All",
    ...new Set(dentistServices.map((ds) => ds.service_name).filter(Boolean)),
  ];
  ...
  const handleFormSubmit = async (formData) => {
    try {
      if (currentDentistService) {
        await updateDentistService({
          dentistServiceID: currentDentistService.id,
          ...formData,
        });
        setAlert({
          show: true,
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteDentistService(itemToDelete.id);
      fetchDentistServices();
      setAlert({
        show: true,
        type: "success",
        title: "Deleted Successfully",
        message: "Service assignment removed from the system.",
      });
    } catch (error) {
      ...
      setAlert({
        ...
        type: "error",
        title: "Delete Failed",
        message: "Failed to remove the service assignment.",
      });
    } finally {
  ...
  const uniqueServiceTypes = [
    "All",
    ...new Set(
      dentistServices.map((ds) => ds.service_type_name).filter(Boolean),
    ),
  ];
  ...
  const filteredDentistServices = dentistServices.filter((ds) => {
    ...
    const matchesCategory =
      selectedFilter === "All" || ds.service_type_name === selectedFilter;

    const matchesService =
      selectedServiceFilter === "All" ||
      ds.service_name === selectedServiceFilter;
                  ...
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-50 to-indigo-50 border border-white shadow-sm flex items-center justify-center text-indigo-600 font-bold text-sm">
                        {getInitials(
                          ds.dentist_first_name,
                          ds.dentist_last_name,
                        )}
                      </div>
                      <div>
                        <p className="font-bold text-slate-900 text-sm">
                          {ds.dentist_first_name} {ds.dentist_last_name}
                        </p>
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200">
                        ...
                        {users
                          .filter((u) => {
                            try {
                              ...
                              return (
                                Array.isArray(roles) &&
                                roles.some((r) => r.includes("DENTIST"))
                              );
                            } catch {
                              return false;
                            }
            ...
            <div className="px-8 py-5 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
              <button
                type="button"
                onClick={handleModalClose}
                className="px-5 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 transition shadow-sm"
              >
                Cancel
              </button>
              <button
                type="submit"
                form="dentistServiceForm"
                className="px-5 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-medium hover:bg-indigo-700 transition shadow-lg shadow-indigo-200"
              >
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-8 text-center">
              ...
              <p className="text-sm text-slate-500 mb-8">
                ...
                <strong>{itemToDelete.service_name}</strong>" from Dr.{" "}
                <strong>
                  {itemToDelete.dentist_first_name}{" "}
                  {itemToDelete.dentist_last_name}
                </strong>
                ?
              </p>
