...
export default function Reminder() {
  ...
  const [isAppointmentsFetched, setIsAppointmentsFetched] =
    useState<boolean>(false);
  ...
  const [currentReminder, setCurrentReminder] = useState<ReminderItem | null>(
    null,
  );
    ...
    if (isModalOpen) {
      ...
      if (currentReminder) {
        ...
        const deepCopy = JSON.parse(
          JSON.stringify(currentReminder.information),
        );
      ...
      setInfoItems([]);
      setAppointments([]); // Clear appointments when modal closes
      setIsAppointmentsFetched(false); // Reset fetch status
    }
  }, [isModalOpen, currentReminder, isAppointmentsFetched]);
  ...
  const handleSlotAction = (
    ...
    setInfoItems((prev) =>
      prev.map((item, i) => {
        ...
        if (action === "add")
          newSlots.push({ startTime: "", endTime: "", message: "" });
        else if (action === "remove" && typeof slotIdx === "number")
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    // Basic Validation
    if (infoItems.some((i) => !i.date))
      return showAlert(
        "warning",
        "Missing Data",
        "All items must have a date.",
      );
    if (
      infoItems.some((i) => i.slots.some((s) => !s.startTime || !s.endTime))
    ) {
      return showAlert(
        "warning",
        "Missing Data",
        "All slots must have start and end times.",
      );
    }
                          ...
                          {r.information.slice(0, 2).map((info, idx) => (
                            ...
                            >
                              ...
                              <span className="text-xs text-slate-500">
                                {info.slots.length} slot
                                {info.slots.length !== 1 ? "s" : ""}
                              </span>
                            </div>
                          ))}
            ...
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 rounded-t-2xl">
              <div>
                <h2 className="text-xl font-bold text-slate-800">
                  {currentReminder
                    ? `Edit Reminder #${currentReminder.id}`
                    : "Create New Reminder"}
                </h2>
                        ...
                        <>
                          <option value="">Select an Appointment</option>
                            {appointments.map((appt) => {
                              console.log(appt)
                              return(<option key={appt.id} value={appt.id}>
                                ID: {appt.id} -{" "}
                                {appt.patient_name
                                  ? `${appt.patient_name} (${appt.appointment_date})`
                                  : appt.date}
                              </option>)
                            })}
                        </>
                ...
                <div className="space-y-6">
                  <div className="flex justify-between items-center">
                    <h3 className="font-bold text-slate-800 flex items-center gap-2">
                      <Calendar size={18} className="text-blue-600" /> Schedule
                      Details
                    </h3>
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <div className="p-4 bg-white space-y-3">
                        {info.slots.map((slot, sIdx) => (
                          <div
                            key={sIdx}
                            className="flex flex-col sm:flex-row gap-2 items-start sm:items-center group"
                          >
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                handleSlotAction(i, "remove", sIdx)
                              }
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white p-6 rounded-xl shadow-xl max-w-sm w-full text-center">
            ...
            <p className="text-sm text-slate-500 mt-2 mb-6">
              Are you sure you want to remove reminder{" "}
            ...
            <div className="flex gap-3 justify-center">
              ...
              <button
                ...
                disabled={isDeleting}
                className="px-4 py-2 bg-rose-600 text-white rounded-lg hover:bg-rose-700 shadow-md font-medium"
              >
