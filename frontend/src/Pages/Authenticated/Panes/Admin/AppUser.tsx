...
import getRoles from "@/API/Authenticated/GetRoles";
import {
  Pencil,
  Trash2,
  Search,
...
export default function AppUser() {
  ...
  const roleOptions = useMemo(() => {
    if (
      ...
      return roleValues.roles[0].map((role) => ({
        label: role.role_name.replace("ROLE_", ""),
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredUsers.map((user) => {
                ...
                return (
                  <tr
                    key={user.id}
                    className={`group transition-colors ${isDisabled ? "bg-slate-50" : "hover:bg-slate-50/50"}`}
                  >
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-4">
                        <div
                          className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold shadow-sm border border-white
                        ...
                        <div className="flex flex-col">
                          <span
                            className={`font-bold text-sm ${isDisabled ? "text-slate-500 line-through decoration-slate-300" : "text-slate-900"}`}
                          >
          ...
          {filteredUsers.map((user) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`absolute top-0 left-0 w-1 h-full ${isDisabled ? "bg-rose-400" : "bg-indigo-500"}`}
                ></div>
                ...
                <div>
                  <div className="flex justify-between items-start mb-4 pl-3">
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-12 h-12 rounded-full flex items-center justify-center text-base font-bold shadow-sm border border-slate-50
                      ...
                      <div>
                        <h3
                          className={`font-bold text-slate-900 ${isDisabled && "line-through text-slate-500"}`}
                        >
                          {user.first_name} {user.last_name}
                        </h3>
                  ...
                  <div className="pl-3 space-y-3 mb-6">
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-slate-600 font-sans truncate max-w-[150px]">
                        {user.email}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden transform transition-all scale-100">
            <div className="p-6 md:p-8 text-center">
              ...
              <p className="text-sm text-slate-500 mb-8 leading-relaxed">
                ...
                .
                <br />
                This action implies removing access immediately and cannot be
                undone.
              </p>
