...
import {
  ...
  updateRole,
  deleteRole,
} from "@/API/Authenticated/admin/RoleAdmin";
import {
  Plus,
  Pencil,
  Trash2,
...
export default function Roles() {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((it) =>
      formatRole(it.role_name).toLowerCase().includes(term)
    );
  }, [items, searchTerm]);
                  ...
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold shadow-sm border border-white bg-gradient-to-br from-indigo-100 to-blue-50 text-indigo-600">
                        {formatRole(it.role_name).slice(0, 1).toUpperCase() || "R"}
                      </div>
                      <div className="flex flex-col">
                        <span className="font-bold text-sm text-slate-900">
                          {formatRole(it.role_name)}
                        </span>
            ...
            <div className="p-6 md:p-8 text-center">
              ...
              <p className="text-sm text-slate-500 mb-8 leading-relaxed">
                ...
                <span className="font-mono bg-slate-100 px-1 rounded text-slate-700">
                  {formatRole(itemToDelete.role_name)}
                </span>
                .
                <br />
