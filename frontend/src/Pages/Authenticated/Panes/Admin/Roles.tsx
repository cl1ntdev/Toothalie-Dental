...
import { getRolesAdmin, createRole, updateRole, deleteRole } from '@/API/Authenticated/admin/RoleAdmin';
...
export default function Roles() {
  ...
  const openEdit = (it: any) => { setEditing(it); setName(it.role_name); setIsModalOpen(true); };

  const save = async () => {
    try {
      if (editing) await updateRole({ id: editing.id, name });
  ...
  const handleDelete = async (id: number) => {
    ...
    try { await deleteRole(id); fetch(); } catch (e) { console.error(e); }
  };
            ...
            <tbody>
              {items.map(it => (
                <tr key={it.id} className="border-t">
                  <td className="px-4 py-3">{it.role_name}</td>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-40 flex items-center justify-center p-4">
            ...
            <div className="bg-white rounded-xl p-6 z-50 w-full max-w-md">
              <h3 className="text-lg font-bold mb-4">{editing ? 'Edit Role' : 'Create Role'}</h3>
              <div>
