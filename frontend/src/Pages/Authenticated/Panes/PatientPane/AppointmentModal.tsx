...
import {
  getServices,
} from "@/API/Authenticated/appointment/GetServices";
      ...
      try {
        ...
        if (serviceRes.data) {
          const uniqueServiceTypes = [
            ...new Set(serviceRes.data.map((s: any) => s.serviceTypeName)),
          ];
        ...
        <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-white shrink-0">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">
              Find a dentist and schedule your visit.
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
          ...
          <div className="w-full lg:w-64 bg-slate-50 border-r border-slate-100 p-6 overflow-y-auto shrink-0 space-y-6">
            ...
            <div className="space-y-3">
              ...
              <label
                className={`flex items-center gap-3 p-3 rounded-xl border transition-all cursor-pointer ${isEmergency ? "bg-rose-50 border-rose-200 ring-1 ring-rose-200" : "bg-white border-slate-200 hover:border-slate-300"}`}
              >
                <Checkbox
                  id="emergency"
                  checked={isEmergency}
              ...
              {appointmentTypes.map((type) => (
                <label
                  key={type.id}
                  className={`flex items-center gap-3 p-3 rounded-xl border transition-all cursor-pointer ${selectedAppointmentTypeId === type.id ? "bg-purple-50 border-purple-200 ring-1 ring-purple-200" : "bg-white border-slate-200 hover:border-slate-300"}`}
                >
                  <input
                    type="radio"
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-start">
                            <div>
                              <h3
                                className={`font-bold text-lg ${isSelected ? "text-indigo-900" : "text-slate-900"}`}
                              >
                                          ...
                                          <div className="flex-1">
                                            ...
                                            >
                                              <PopoverTrigger asChild>
                                                <Button
                                                  variant="outline"
                                                  className={`w-full justify-start text-left h-10 text-sm border-slate-200 ${!date && "text-slate-400"}`}
                                                >
