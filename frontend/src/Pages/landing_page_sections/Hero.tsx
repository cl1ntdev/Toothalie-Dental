    ...
    <section className="relative min-h-[100svh] flex flex-col items-center justify-center overflow-hidden pt-10 md:pt-20 font-poppins isolate bg-white">
      ...
      <div className="relative z-30 mt-20 md:mt-24 mb-10 md:mb-0 flex flex-col items-center w-full max-w-5xl">
        ...
        <motion.div 
          ...
          transition={{ opacity: { delay: 1.8, duration: 2 }, rotate: { duration: 40, repeat: Infinity, ease: "linear" } }}
        ...
        <motion.div 
          initial={{ opacity: 0 }}
          animate={{ opacity: 1, rotate: -360 }}
          transition={{ opacity: { delay: 1.8, duration: 2 }, rotate: { duration: 50, repeat: Infinity, ease: "linear" } }}
        ...
        <motion.div
          ...
          transition={{ duration: 1.5, delay: 1.2, type: "spring", bounce: 0.3 }}
        ...
        >
          ...
          >
            <motion.div variants={reverseFloatAnimation} initial="initial" animate="animate">
              <div className="glass-panel rounded-2xl p-4 md:p-6 flex flex-col items-start gap-1 transform transition-all hover:-translate-y-2 hover:shadow-blue-400/40 cursor-default">
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-3xl md:text-4xl font-extrabold text-gray-800 font-['Plus_Jakarta_Sans'] flex items-baseline tracking-tight">
                    <CountUp from={0} to={15} separator="," direction="up" duration={2.5} startCounting={true} />
                    <span className="text-blue-600 ml-1">k+</span>
                  </div>
                </div>
                <p className="text-xs md:text-sm text-gray-500 font-semibold uppercase tracking-wider mt-1">Happy Smiles Restored</p>
              </div>
            </motion.div>
          </motion.div>
          ...
          <motion.div 
            initial={{ opacity: 0, x: 50, scale: 0.8 }}
            animate={{ opacity: 1, x: 0, scale: 1 }}
            transition={{ duration: 0.8, delay: 2.4, type: "spring", bounce: 0.4 }}
      ...
      <div className="absolute left-6 lg:left-12 top-[45%] max-w-[180px] lg:max-w-[240px] hidden xl:block z-20">
        <motion.div initial={{ width: 0 }} animate={{ width: 60 }} transition={{ duration: 1, delay: 2.8 }} className="h-[3px] bg-blue-500 mb-5 rounded-full shadow-[0_0_10px_rgba(59,130,246,0.5)]" />
        <TypingText text="Are you ready for premium care?" className="text-2xl lg:text-3xl text-gray-800 font-semibold mb-2 leading-snug font-['Plus_Jakarta_Sans']" delay={3.2} />
      </div>
      ...
      <div className="absolute right-6 lg:right-12 top-[45%] max-w-[180px] lg:max-w-[240px] hidden xl:block text-right z-20 flex flex-col items-end">
        <motion.div initial={{ width: 0 }} animate={{ width: 60 }} transition={{ duration: 1, delay: 3.0 }} className="h-[3px] bg-teal-400 mb-5 rounded-full ml-auto shadow-[0_0_10px_rgba(45,212,191,0.5)]" />
        <TypingText text="We're open to new patients." className="text-2xl lg:text-3xl text-gray-800 font-semibold mb-2 leading-snug font-['Plus_Jakarta_Sans']" delay={3.4} />
      </div>

      {/* 6. Footer Info */}
      <div className="absolute bottom-6 left-6 right-6 justify-between items-end hidden md:flex z-20">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 3.6 }}>
          <p className="text-[10px] text-gray-400 uppercase tracking-[0.2em] font-bold mb-1">Powered by</p>
          <p className="text-base font-extrabold text-gray-900 font-['Plus_Jakarta_Sans'] tracking-tight">Toothalie</p>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 3.7 }} className="text-right">
          <p className="text-[10px] text-gray-400 uppercase tracking-[0.2em] font-bold mb-1">Platform</p>
          <p className="text-base font-extrabold text-gray-900 tracking-tight">© 2026</p>
        </motion.div>
      </div>
