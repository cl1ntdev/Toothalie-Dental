...
class DentistService extends AbstractController
{
    ...
    #[
        Route(
            "/api/admin/get-dentist-service",
            name: "get-dentist-service",
            methods: ["POST"],
        ),
    ]
    public function getOneDentistService(
        Request $req,
    ...
    #[
        Route(
            "/api/admin/delete-dentist-service",
            name: "delete-dentist-service",
            methods: ["POST"],
        ),
    ]
    public function deleteDentistService(
        Request $req,
        Connection $connection,
    ...
    #[
        Route(
            "/api/admin/update-dentist-service",
            name: "update-dentist-service",
            methods: ["POST"],
        ),
    ]
    public function updateDentistService(
        Request $req,
        Connection $connection,
        ActivityLogger $logger,
    ): JsonResponse {
        try {
            ...
            $originalService = $connection->fetchAssociative(
                "SELECT * FROM dentist_service WHERE id = ?",
                [$dentistServiceID],
            );
            ...
            foreach ($updateData as $key => $newValue) {
                $oldValue = $originalService[$key] ?? null;
                if ($oldValue != $newValue) {
                    $changedFields[] = "{$key} ('{$oldValue}' to '{$newValue}')";
                }
            }

            $connection->update("dentist_service", $updateData, [
                "id" => $dentistServiceID,
            ]);
            ...
            if (!empty($changedFields)) {
                $logger->log(
                    ...
                    "Admin updated dentist service ID {$dentistServiceID}. Changes: " .
    ...
    #[
        Route(
            "/api/admin/create-dentist-service",
            name: "create-dentist-service",
            methods: ["POST"],
        ),
    ]
    public function createDentistService(
        Request $req,
        Connection $connection,
        ActivityLogger $logger,
    ): JsonResponse {
            ...
            if (!$serviceExists) {
                return new JsonResponse(
                    [
                        "status" => "error",
                        "message" =>
                            "Invalid service_id, service does not exist",
                    ],
                    400,
                );
            }
